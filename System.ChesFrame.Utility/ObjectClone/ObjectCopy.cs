 ...
 * Info:
 * 
 * 对象DeepCopy采用Emit命令方式，效率高
 * 
 * 目前能支持任何带无参数的构造函数的类的深拷贝，
 * 一元数组的深拷贝，数组和类的循环嵌套深拷贝，
 * 双向链表结构的类型对象深拷贝，进行对象拷贝过程中，
 * 未判断任何附加特性，因此可序列化标志对其无效，
 * 即使不可序列化的对象，也可以进行深拷贝。
 * From:http://blog.csdn.net/qldsrx/article/details/8265985
 */
